...
function GradeBar({ grade, count, maxCount }) {
  const pct = maxCount > 0 ? Math.max((count / maxCount) * 100, 0) : 0;
...
function RecentStudent({ student, onClick }) {
  return (
    <li
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[12px] md:text-[13px] font-medium truncate">{student.name}</div>
        <div className="text-[10px] md:text-[11px] text-[var(--color-text-muted)]">Grade {student.grade}</div>
      </div>
      <span className="font-bold text-[12px] md:text-[13px] text-[var(--color-secondary)] flex-shrink-0 tabular-nums">{student.gpa.toFixed(2)}</span>
    </li>
  );
}
...
function ActivityItem({ activity }) {
  ...
  return (
    <div className="flex items-start gap-2 md:gap-2.5 py-1.5 md:py-2 border-b border-[var(--color-border)] last:border-b-0 text-[12px] md:text-[13px]">
      <span className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${dotColor}`} />
      <span className="text-[var(--color-text)]">{activity.text}</span>
      <span className="ml-auto text-[10px] md:text-[11px] text-[var(--color-text-dim)] whitespace-nowrap flex-shrink-0">{activity.time}</span>
    </div>
  );
}
...
export default function Dashboard({ onStudentClick }) {
  ...
  const maxGradeCount = Math.max(...Object.values(stats.byGrade), 1);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-3 md:gap-3.5">
        ...
        >
          ...
          <div className="flex flex-col gap-2.5 md:gap-3">
            {[9, 10, 11, 12].map((g) => (
              <GradeBar key={g} grade={g} count={stats.byGrade[g]} maxCount={maxGradeCount} />
            ))}
          </div>
        </div>
        ...
        >
          ...
          {recent.length ? (
            <ul className="flex flex-col gap-1.5 md:gap-2">
              {recent.map((st) => (
                <RecentStudent key={st.id} student={st} onClick={onStudentClick} />
              ))}
            </ul>
          ) : (
